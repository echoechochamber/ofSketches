...
void ofApp::setup(){
    ...
    trainingData.setInputAndTargetDimensions( trainingInputs, 3 );
    ...
    // Setup tracker - WHY DO I HAVE TO DO THE ABSOLUTE PATH?
    tracker.setup("/Users/akwilliams/Documents/CODE/openFrameworks/addons/ofxFaceTracker2/model/shape_predictor_68_face_landmarks.dat");
    
}

//--------------------------------------------------------------
void ofApp::update(){
    float * messageTest = new float[3]{1.0,2.0,3.0};
    sendOscMessage(messageTest);
...
//        }
        ...
        if( trainingModeActive){
            ...
            if( recordTrainingData ){
                ...
                // NOTE! here is where you add the vecotr asmple to the training data
                // this is where you need to make the
                
                VectorFloat targetVector(3);
                targetVector[0] = rValue;
                targetVector[1] = gValue;
                targetVector[2] = bValue;
...
///* Not here is where we can get the data frokm the trainin g*/
//                float tmpArr[] = {pipeline.getRegressionData()[0], pipeline.getRegressionData()[1], pipeline.getRegressionData()[2]};
//                sendOscMessage(tmpArr);
//            }else{
...
void ofApp::draw(){
    ...
    ofDrawBitmapString("Red: " + ofToString(rValue) , ofGetWidth()-100, 30);
    ofDrawBitmapString("Green: " + ofToString(gValue), ofGetWidth()-100, 50);
    ofDrawBitmapString("Blue: " + ofToString(bValue) , ofGetWidth()-100, 70);
...
void ofApp::keyPressed(int key){
    ...
    switch ( key) {
        ...
        case OF_KEY_UP:
            if(editNum == 1 && (rValue < 255.0)){
                rValue += 15.0;
            } else if(editNum ==2 && (gValue < 255.0) ){
                gValue += 15.0;
            } else if(editNum == 3 && (bValue < 255.0) ){
                bValue += 15.0;
            }
            break;
        case OF_KEY_DOWN:
            if(editNum == 1 && (rValue > 0)){
                rValue -= 15.0;
            } else if(editNum ==2 && (gValue > 0) ){
                gValue -= 15.0;
            } else if(editNum == 3 && (bValue > 0) ){
                bValue -= 15.0;
            }
            break;
        case OF_KEY_TAB:
...
//--------------------------------------------------------------
void ofApp::sendOscMessage(float arr[]){
    ofxOscMessage m;
    m.setAddress( "/test" );
    for(int i =0; i < 3; i++){
        m.addFloatArg( arr[i] );
